...
| **attribute runtime** — `#[memo]`/`#[cache(ttl)]`, `#[self_healing]`/`#[retry]`, `#[budget]`/`#[cost]` (+`budget_of`), `#[instrument]` (+`instrument_of`), `#[profile]` (+`profile_of`), `#[requires]`/`#[ensures]`/`#[assumes]` contracts, `#[trace]`/`#[log]`/`#[audit]`, `#[time_travel]` (+`history_of`), `#[experimental]`/`#[since]` warnings, `#[hot_swap]` (+`hot_swap`), `#[integrity]`/`#[anti_tamper]` (+`integrity_of`), `snapshot`/`rollback`, `#[zero_alloc]` (checked) | Run ✅ — uniform on all tiers; `tests/corpus/attributes.nova` + `attr_runtime.nova`, byte-identical incl. the Nova self-host evaluator |
